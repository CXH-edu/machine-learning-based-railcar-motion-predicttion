...
# 自定义XGBoost训练函数以实现带阈值的早停
def train_xgboost_with_threshold(params, X_train, y_train, X_valid, y_valid, max_iter=1000, patience=10, min_delta=1e-4):
    ...
    for i in range(1, max_iter + 1):
        ...
        # 计算 RMSE
        train_pred = current_model.predict(dtrain)
        valid_pred = current_model.predict(dvalid)
...
if __name__ == "__main__":
    ...
    for i, param_values in enumerate(param_combinations):
        ...
        params = {
            ...
            'min_child_weight': param_values[2],
        ...
        # 训练模型
        model, early_stop_iter, train_rmse_history, valid_rmse_history = train_xgboost_with_threshold(
            params, X_train, y_train, X_valid, y_valid, max_iter=1000, patience=10, min_delta=1e-4
        )
    ...
    y_pred_normalized = best_model.predict(dtest)
    ...
    y_pred_train_original = scaler_y.inverse_transform(
        best_model.predict(dtrain).reshape(-1, 1)
    )
    ...
    y_pred_valid_original = scaler_y.inverse_transform(
        best_model.predict(dvalid).reshape(-1, 1)
    )
    ...
    output_file_path = os.path.join(
        output_dir, f"XGBoost_{targets[0]}_Metrics_{current_time}.csv"
    )
    ...
    plt.title("XGBoost 实际值 vs. 预测值 (原始尺度)")
    plt.grid(True)
    plt.show()
    df = pd.DataFrame(
        {
    ...
    output_file_path = os.path.join(
        output_dir, f"XGBoost_{targets[0]}_Predictions_{current_time}.csv"
    )
    df.to_csv(output_file_path, index=False, encoding="utf-8-sig")
    ...
    plt.xlabel("迭代次数")
    plt.ylabel("RMSE")
    plt.title("XGBoost 最优模型的训练和验证集 RMSE")
    plt.legend()
    plt.grid(True)
    plt.show()
    ...
    output_file_path = os.path.join(
        output_dir, f"XGBoost_{targets[0]}_RMSE_History_{current_time}.csv"
    )
    df.to_csv(output_file_path, index=False, encoding="utf-8-sig")
    ...
    output_file_path = os.path.join(
        output_dir, f"XGBoost_{targets[0]}_Parameters_{current_time}.csv"
    )
    df.to_csv(output_file_path, index=False, encoding="utf-8-sig")
