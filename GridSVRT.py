...
features = [
    ...
    "T12", "TheoreticalT12", "TheoreticalT23", "GravityCenter3"
]
targets = ["T23"]
...
if __name__ == "__main__":
    ...
    for i, values in enumerate(param_combinations):
        params = {"C": values[0], "epsilon": values[1], "kernel": values[2]}
        ...
        print(f"第{i+1}/{len(param_combinations)}组: {params}, 验证集RMSE: {valid_rmse:.4f}")
    ...
    df_metrics.to_csv(os.path.join(output_dir, f"SVR_{targets[0]}_Metrics_{current_time}.csv"), index=False, encoding="utf-8-sig")

    df_pred = pd.DataFrame({
        f"实际 {targets[0]}": y_test_orig.flatten(),
        f"预测 {targets[0]}": y_test_pred_orig.flatten()
    })
    df_pred.to_csv(os.path.join(output_dir, f"SVR_{targets[0]}_Predictions_{current_time}.csv"), index=False, encoding="utf-8-sig")
    ...
    df_params.to_csv(os.path.join(output_dir, f"SVR_{targets[0]}_Parameters_{current_time}.csv"), index=False, encoding="utf-8-sig")
