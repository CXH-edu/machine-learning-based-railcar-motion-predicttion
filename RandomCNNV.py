...
if __name__ == "__main__":
    # 超参数空间定义
    param_dist = {
        'learning_rate': uniform(0.001, 0.009),  # [0.001, 0.01]
        'conv_layers': {
            'num_layers': randint(1, 3),  # 1 or 2 layers
            'filters': uniform(16, 112),  # [16, 128]
            'kernel_size': [3, 5, 7]  # Extended from [3]
        },
        'dense_layers': {
            'num_layers': randint(1, 3),  # 1 or 2 layers
            'units': uniform(32, 224)  # [32, 256]
        },
        'batch_size': uniform(16, 112),  # [16, 128]
        'dropout_rate': uniform(0.0, 0.2)  # [0.0, 0.2]
    }
    ...
    print("开始进行随机超参数搜索..., 开始时间:", start_time)
    ...
    random.seed(42)  # 确保随机采样的可重复性
    np.random.seed(42)  # 为 np.random.uniform 和 randint 设置种子

    for i in range(n_iterations):
        iteration_start_time = pd.Timestamp.now()
        # 随机生成卷积层
        num_conv_layers = param_dist['conv_layers']['num_layers'].rvs()
        conv_layers = []
        for _ in range(num_conv_layers):
            filters = int(param_dist['conv_layers']['filters'].rvs())
            kernel_size = random.choice(param_dist['conv_layers']['kernel_size'])
            conv_layers.append((filters, kernel_size))

        # 随机生成全连接层
        num_dense_layers = param_dist['dense_layers']['num_layers'].rvs()
        dense_layers = []
        for _ in range(num_dense_layers):
            units = int(param_dist['dense_layers']['units'].rvs())
            dense_layers.append(units)

        params = {
            'learning_rate': param_dist['learning_rate'].rvs(),
            'conv_layers': conv_layers,
            'conv_num_layers': num_conv_layers,
            'dense_layers': tuple(dense_layers),
            'dense_num_layers': num_dense_layers,
            'batch_size': int(param_dist['batch_size'].rvs()),
            'dropout_rate': param_dist['dropout_rate'].rvs(),
        }

        # 记录超参数，格式参考 hidden_layer_sizes
        conv_layers_str = str([num_conv_layers, [
            (filters, kernel_size) for filters, kernel_size in params['conv_layers']
        ] + [(None, None)] * (3 - num_conv_layers)][:3])
        dense_layers_str = str([num_dense_layers, [
            units for units in params['dense_layers']
        ] + [None] * (3 - num_dense_layers)][:3])

        learning_rates.append(params['learning_rate'])
        # 替换后（CNN版本）
        conv_layers_list.append(conv_layers_str)
        dense_layers_list.append(dense_layers_str)
    ...
    # 格式化 conv_layers 和 dense_layers，参考 hidden_layer_sizes
    conv_layers_str = str([best_params['conv_num_layers'], [    (filters, kernel_size) for filters, kernel_size in best_params['conv_layers']
    ] + [(None, None)] * (3 - best_params['conv_num_layers'])][:3])
    dense_layers_str = str([best_params['dense_num_layers'], [    units for units in best_params['dense_layers']
    ] + [None] * (3 - best_params['dense_num_layers'])][:3])
    ...
    df = pd.DataFrame(
        {
            ...
            "conv_layers": [conv_layers_str],
            "dense_layers": [dense_layers_str],
    ...
    output_file_path = os.path.join(
        output_dir, f"CNN_RandomSearch_{targets[0]}_Metrics_{current_time}.csv"
    )
    ...
    output_file_path = os.path.join(
        output_dir, f"CNN_RandomSearch_{targets[0]}_Predictions_{current_time}.csv"
    )
    df.to_csv(output_file_path, index=False, encoding="utf-8-sig")
    ...
    output_file_path = os.path.join(
        output_dir, f"CNN_RandomSearch_{targets[0]}_RMSE_History_{current_time}.csv"
    )
    df.to_csv(output_file_path, index=False, encoding="utf-8-sig")
    ...
    output_file_path = os.path.join(
        output_dir, f"CNN_RandomSearch_{targets[0]}_Parameters_{current_time}.csv"
    )
    df.to_csv(output_file_path, index=False, encoding="utf-8-sig")
