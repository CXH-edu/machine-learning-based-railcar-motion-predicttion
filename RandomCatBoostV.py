...
if __name__ == "__main__":
    # 定义超参数范围
    param_dist = {
        "learning_rate": uniform(loc=0.005, scale=0.095),  # [0.005, 0.1]
        "depth": randint(4, 9),                            # [4, 8]
        #"min_data_in_leaf": randint(2, 11),  # [2, 10]
        "l2_leaf_reg": randint(1, 6),                      # [1, 5]
    ...
    for i in range(n_iterations):
        ...
        params = {
            ...
            "depth": param_dist["depth"].rvs(),
            #"min_data_in_leaf": param_dist["min_data_in_leaf"].rvs(),
            "l2_leaf_reg": param_dist["l2_leaf_reg"].rvs(),
    ...
    output_file_path = os.path.join(
        output_dir, f"CatBoost_RandomSearch_{targets[0]}_Metrics_{current_time}.csv"
    )
    ...
    plt.title("CatBoost实际值 vs. 预测值 (原始尺度)")
    plt.grid(True)
    plt.show()
    df = pd.DataFrame(
        {
    ...
    output_file_path = os.path.join(
        output_dir, f"CatBoost_RandomSearch_{targets[0]}_Predictions_{current_time}.csv"
    )
    df.to_csv(output_file_path, index=False, encoding="utf-8-sig")
    ...
    plt.xlabel("迭代次数")
    plt.ylabel("RMSE")
    plt.title("CatBoost最优模型的训练和验证集 RMSE")
    plt.legend()
    plt.grid(True)
    plt.show()
    ...
    output_file_path = os.path.join(
        output_dir, f"CatBoost_RandomSearch_{targets[0]}_RMSE_History_{current_time}.csv"
    )
    df.to_csv(output_file_path, index=False, encoding="utf-8-sig")
    ...
    output_file_path = os.path.join(
        output_dir, f"CatBoost_RandomSearch_{targets[0]}_Parameters_{current_time}.csv"
    )
    df.to_csv(output_file_path, index=False, encoding="utf-8-sig")
