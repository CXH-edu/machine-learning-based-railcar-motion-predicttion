...
# 自定义LightGBM训练函数以实现带阈值的早停
def train_lightgbm_with_threshold(params, X_train, y_train, X_valid, y_valid, max_iter=1000, patience=10, min_delta=1e-4):
...
if __name__ == "__main__":
    ...
    for i in range(n_iterations):
        ...
        params = {
            ...
            'num_leaves': param_dist['num_leaves'].rvs(),
        ...
        # 训练模型
        model, early_stop_iter, train_rmse_history, valid_rmse_history = train_lightgbm_with_threshold(
    ...
    output_file_path = os.path.join(
        output_dir, f"LightGBM_RandomSearch_{targets[0]}_Metrics_{current_time}.csv"
    )
    ...
    plt.title("LightGBM 实际值 vs. 预测值 (原始尺度)")
    plt.grid(True)
    plt.show()
    ...
    output_file_path = os.path.join(
        output_dir, f"LightGBM_RandomSearch_{targets[0]}_Predictions_{current_time}.csv"
    )
    df.to_csv(output_file_path, index=False, encoding="utf-8-sig")
    ...
    plt.xlabel("迭代次数")
    plt.ylabel("RMSE")
    plt.title("LightGBM 最优模型的训练和验证集 RMSE")
    plt.legend()
    plt.grid(True)
    plt.show()
    ...
    output_file_path = os.path.join(
        output_dir, f"LightGBM_RandomSearch_{targets[0]}_RMSE_History_{current_time}.csv"
    )
    df.to_csv(output_file_path, index=False, encoding="utf-8-sig")
    ...
    output_file_path = os.path.join(
        output_dir, f"LightGBM_RandomSearch_{targets[0]}_Parameters_{current_time}.csv"
    )
    df.to_csv(output_file_path, index=False, encoding="utf-8-sig")
