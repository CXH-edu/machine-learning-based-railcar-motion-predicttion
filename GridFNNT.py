...
X_train, X_valid, y_train, y_valid = train_test_split(
    X_temp, y_temp, test_size=0.17647, random_state=40
)

# 自定义FNN训练函数以实现带阈值的早停
def train_fnn_with_threshold(params, X_train, y_train, X_valid, y_valid, max_iter=200, patience=10, min_delta=1e-4):
    # 构建模型
...
if __name__ == "__main__":
    ...
    for i, param_values in enumerate(param_combinations):
        ...
        params = {
            ...
            'hidden_layer_sizes': param_values[1],
            'batch_size': param_values[2],
            'dropout_rate': param_values[3],
        }
        learning_rates.append(params['learning_rate'])
        hidden_layer_sizes_list.append(str(params['hidden_layer_sizes']))
        ...
        model, early_stop_epoch, train_rmse_history, valid_rmse_history = train_fnn_with_threshold(
            params, X_train, y_train, X_valid, y_valid, max_iter=200, patience=10, min_delta=1e-4
        )
    ...
    output_file_path = os.path.join(
        output_dir, f"FNN_{targets[0]}_Metrics_{current_time}.csv"
    )
    ...
    output_file_path = os.path.join(
        output_dir, f"FNN_{targets[0]}_Predictions_{current_time}.csv"
    )
    df.to_csv(output_file_path, index=False, encoding="utf-8-sig")
    ...
    output_file_path = os.path.join(
        output_dir, f"FNN_{targets[0]}_RMSE_History_{current_time}.csv"
    )
    df.to_csv(output_file_path, index=False, encoding="utf-8-sig")
    ...
    output_file_path = os.path.join(
        output_dir, f"FNN_{targets[0]}_Parameters_{current_time}.csv"
    )
    df.to_csv(output_file_path, index=False, encoding="utf-8-sig")
